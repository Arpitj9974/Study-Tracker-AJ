/* ============================================================================
 * dashboard-cuet-pg.js — Dashboard controller for CUET PG General (COQP11)
 * ============================================================================ */

function renderCUETPGDashboard() {
  const config = EXAM_CONFIG_CUET_PG_GENERAL;
  const priorityData = CUET_PG_GENERAL_PRIORITY_DATA;

  localStorage.setItem('selectedExam', 'cuet_pg_general');
  ...
  const subjectCardsHTML = config.subjects.map(subj => {
    ...
    return `
      <div style="background:var(--bg-elevated);border:1px solid var(--border-medium);border-radius:12px;padding:20px;display:flex;flex-direction:column;justify-content:space-between">
        ...
        <a href="tracker-cuet-pg.html?subj=${subj.key}" class="btn" style="background:${subj.color};text-align:center;text-decoration:none;display:block;padding:10px;font-size:13px;border-radius:8px;color:white;font-weight:600">
  ...
  document.getElementById('cpg-subject-cards').innerHTML = subjectCardsHTML;
  ...
  if (donutPath) {
    ...
    donutPath.setAttribute('stroke', '#059669');
  }

  // Days Countdown
  const daysLeft = Math.ceil((new Date(config.examDate + 'T00:00:00') - new Date()) / 86400000);
  document.getElementById('cpg-days-left').innerHTML = `
    <strong>Target Date:</strong> 20 Mar 2027 (${daysLeft > 0 ? daysLeft + ' days left' : 'Exam Day!'})
  `;

  // Pattern Details
  const p = config.examPattern;
  document.getElementById('cpg-pattern-details').innerHTML = `
    <div style="margin-bottom:8px"><strong>Mode:</strong> ${p.mode}</div>
    <div style="margin-bottom:8px"><strong>Paper Specs:</strong> 75 MCQs · 300 Marks · 90 Mins (+4 / −1)</div>
    <div style="margin-bottom:8px"><strong>Sections:</strong> Quant (~20 Qs), GK & CA (~20 Qs), Logical Reasoning (~15 Qs), English (~15 Qs), Computer Basics (~5 Qs)</div>
    <div><strong>Programs Covered:</strong> MBA, DU LLB, MA Social Work, Journalism & Public Administration across Central & State Universities</div>
  `;
}

document.addEventListener('DOMContentLoaded', () => {
  renderCUETPGDashboard();
});

window.addEventListener('cloudDataSynced', renderCUETPGDashboard);
window.addEventListener('storage', renderCUETPGDashboard);
